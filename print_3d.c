...
/*   Updated: 2020/10/11 00:06:36 by lbagg            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_3d(t_all *all, float dist, int num_ray, int text_x)
{
	...
	{
		pos_y = ((float)line_start - ((float)SCREEN_HEIGHT / 2 - (float)line_height / 2)) / (float)line_height;
		// printf("%f\n", pos_y);
		pos_y *= (float)all->texture->height;
		printf("%f\n", pos_y);
		color = (all->texture->addr + (((int)pos_y * all->texture->line_length) + (text_x * (all->texture->bits_per_pixel / 8))));
